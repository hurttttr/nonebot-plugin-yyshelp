...
async def construct_change_waifu_msg(member_info: dict, user_info: dict,new_waifu_id: int, bot_id: int, times: int,
    ...
    if new_waifu_id == bot_id:
        return MessageSegment.text(
            f'\n你今天的群友老婆是我哦~'
            f'\n如果你这个{text1}敢抛弃我的话，你今天就没老婆了哦') + MessageSegment.image(
            img)
    msg = MessageSegment.text('')
    if times == limit_times - 1:
        msg = MessageSegment.text(f'\n{text1}，再换你今天就没{text2}了！')
    return msg + MessageSegment.text(f'\n你今天的群友{text2}是：') + MessageSegment.image(img) + MessageSegment.text(
        f'{member_name}({new_waifu_id})')


async def construct_waifu_msg(member_info: dict, user_info: dict, waifu_id: int, bot_id: int, is_first: bool) -> Message:
    """
    构造发送信息
    """
    #性别判断
    ...
    return MessageSegment.text(f'\n你今天已经有{text2}了，要好好对待她哦~') + MessageSegment.image(
