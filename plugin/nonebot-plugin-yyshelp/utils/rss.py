...
class RssData:
    ...
    def __init__(self, author, description, pubDate, link, href_links,
                 src_links):
...
async def get_rssdataList(url: str, keyword: str = '',number:int = 5) -> list[RssData]:
    ...
    for elem in root[0].iter(tag='item'):  #获取rss中item的内容
        ...
        if (keyword := keyword.strip()) and keyword not in description:
            continue
        pubDate = datetime.strptime(elem[2].text,
                                    "%a, %d %b %Y %H:%M:%S %Z")  #更新时间，并进行格式转换
        link = elem[4].text  #原链接

        #获取详细内容中的链接
        href_links = re.findall(r'href="([^"]*)"',
                                description)  #href链接，一般为网页，视频等
