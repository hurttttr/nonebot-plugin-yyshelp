...
def get_or_update_icon() -> list[Heros]:
    ...
    for key, value in rarity.items():
        # 判断文件夹是否存在
        if not os.path.exists("icon/" + value):
            os.makedirs("icon/" + value)  # 递归创建文件夹
            url = f"https://g37simulator.webapp.163.com/get_heroid_list?callback=jQuery1113002988038468302756_1719390741330&rarity={key}&interactive=0&material_type=0&page=1&per_page=200"
            response = requests.get(url, headers=headers, timeout=10)
            data = json.loads(response.text[43:-1])["data"]

            for heroid in data:
                # print(heroid, data[heroid]['name'])
                icon_url = f"https://yys.res.netease.com/pc/zt/20161108171335/data/shishen/{heroid}.png?v5"
        ...
        # 若存在则判断是否有更新
            ...
            for heroid in data:
                heros_list.append(Heros(heroid, data[heroid]["name"], value))
                # 检查指定路径下是否存在该式神的图标文件
                if not os.path.exists(f"icon/{value}/{heroid}.png"):
                    # 打印新增式神的提示信息
