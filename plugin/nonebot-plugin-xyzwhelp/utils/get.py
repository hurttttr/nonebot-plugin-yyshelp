...
def get_image_and_cut(url: str) -> None:
    ...
    # 裁剪图像的下部区域，转换为灰度并保存
    image.crop((0, height * 0.75, width, height * 0.87)).convert("L").save(
        r"temp/cut2.jpg"
    )


def ocr_space_file(
    filename, overlay=False, api_key=config.xyzwhelp_apikey, language="eng"
) -> str:
    """
...
def process_data(data: str) -> int:
    ...
    # 替换所有的'l', 'L', 'I', 'i'为'1'
    data = data.replace("l", "1").replace("L", "1").replace("I", "1").replace("i", "1")
    # 将处理后的字符串转换为整数并返回
    return int(data)
...
def get_return_str() -> str:
    ...
    """
    # 从'cut1.jpg'图片中提取预编码
    result1 = json.loads(ocr_space_file(r"temp/cut1.jpg", language="chs"))[
        "ParsedResults"
    ][0]["ParsedText"]
    pre_code = re.findall(r"\d+", result1)[0].split("/")[0]

    # 从'cut2.jpg'图片中提取四种材料的信息
    result2 = re.findall(
        r"X[\da-zA-Z]+",
        json.loads(ocr_space_file(r"temp/cut2.jpg"))["ParsedResults"][0]["ParsedText"],
    )
